...
### 5.3 亮 / 暗双主题适配（重点） {: #svg-dark-theme }
...
暗色主题下的颜色处理：若用内联法，按 [第 5.3 节](#svg-dark-theme) 用 `currentColor`；若用引用法，要么准备两套配色，要么接受固定配色。
...
### 12.0 拿到现成 SVG 怎么微调（改别人的 / 改 AI 生成的）

很多时候你不是从零画，而是拿到一份别人或 AI 给的 SVG，想做点小调整。掌握下面三招，不动脑也能改：

1. **文字太挤 / 重叠了？**
   找到对应的 `<text>` 或 `<tspan>`，把 `y` 调大 `5~10px`（往下移），或把 `font-size` 缩小 1 个字号（如 `11` 改 `10`）。多行文字则调 `dy`（见 3.3 节）。
2. **内容超出边界 / 想加高画布？**
...
> 这三招整理自一份「SVG 快速修改指南」的实战经验；本文已按普适 / 科普约定改写，去掉了原版的对话口吻。
> 若这份 SVG 要放进会切换亮 / 暗主题的站点，记得把写死的颜色（如 `#5f6368`）改成 `currentColor`（见第 5.3 节），否则暗色背景下可能糊掉。

!!! tip "要系统学「怎么改一张现成的图」，看这里"
    上面三招只覆盖最常见的微调。若你需要的是**完整的改图能力**——怎么读懂一张陌生的 SVG、怎么改才不会牵一片、怎么让图在亮暗主题下都好看、出问题怎么排错——
    请移步本站的 **[SVG 动手区](../前端设计/SVG/index.md)**（在 *技术与编程 → 前端设计* 下）。

    两处的分工：**本文是语法字典（怎么写），那边是动手手册（怎么改）**。
