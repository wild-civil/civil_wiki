...
> **响应式技巧**：在网页里嵌入时，用 `max-width:100%; height:auto` 让图随容器宽度缩放即可。注意 **`.svg` 文件本身要保留 `width`/`height`** 提供固有尺寸——如果只留 `viewBox` 而删掉它们，浏览器在 `<img>` 下会把高度算成 0，图反而看不见（详见 [亮暗双主题与 MkDocs 嵌入](亮暗双主题与MkDocs嵌入.md) 的「尺寸控制」）。
...
相关：[← SVG 动手区](/技术与编程/前端设计/SVG/) · [下一步：六种常见手术 →](改图的六种常见手术.md) · [术语速查](SVG术语速查.md) · [语法字典](./SVG从认识到制作.md)
