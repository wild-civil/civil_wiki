...
![复制相近元素并改坐标，加出一个新模块](assets/svg-op-add.svg){: style="max-width:680px;width:100%;height:auto"}
...
!!! warning "最容易漏的：一个视觉元素 = 多个 SVG 元素"
    ...
    **做法**：复制时先数清楚目标由几个元素构成（回看 [读图第 ③ 步](SVG入门_读图与制作.md#step3-locate)），每个都要改。
...
![删掉元素后留下空洞，其余元素纹丝不动](assets/svg-op-del.svg){: style="max-width:680px;width:100%;height:auto"}
...
![linecap=round 端点变圆、linejoin=round 拐角变柔](assets/svg-op-attr.svg){: style="max-width:680px;width:100%;height:auto"}
...
![套 g transform 后整组一次性平移，子元素坐标不变](assets/svg-op-group.svg){: style="max-width:680px;width:100%;height:auto"}
...
![y 是基线；dominant-baseline=middle 让文字垂直居中](assets/svg-op-text.svg){: style="max-width:680px;width:100%;height:auto"}
...
![defs 定义一次，use 多处引用，改定义即全部生效](assets/svg-op-reuse.svg){: style="max-width:680px;width:100%;height:auto"}
...
相关：[← SVG 入门：读图与制作](SVG入门_读图与制作.md) · [术语速查](SVG术语速查.md) · [下一步：亮暗双主题与嵌入 →](亮暗双主题与MkDocs嵌入.md) · [排错清单](SVG排错清单.md)
