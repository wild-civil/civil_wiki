...
我目前手上有**五个站点**，分别用**三种引擎**搭建，现在都并排在 `wild-civil/` 工作区里统一管理。这篇文章把它们的**目录结构、改哪里、怎么编译、怎么在本地看**一次讲清，免得我自己过段时间也忘了。

> 想看更直白的「改哪里、敲哪行命令」版本，先看 [全站架构小白篇 →](建站笔记-全站架构小白篇.md)；想看引擎内部怎么运作，看 [全站架构进阶篇 →](建站笔记-全站架构进阶篇.md)。
...
MkDocs 是**内容驱动**：你写 Markdown，它按 `mkdocs.yml` 把文章织成站点。本站最大、最常用的就是它（你现在读的这篇就在它里面）。
...
- **加一篇笔记**：在 `docs/` 对应分区文件夹新建 `xxx.md`，再到 `mkdocs.yml` 的 `nav:` 里加一行指向它：
...
- **改站点级配置**：编辑 `mkdocs.yml`（`nav` / `theme` / `plugins` / `extra_css` / `markdown_extensions`）。
...
> ⚠️ **大坑**：`nav` 里引用的文件必须**真实存在**，否则 CI 构建直接报错、`gh-deploy` 不执行，结果**整站不更新**（曾经就因为 nav 引用了未提交的文件，导致新章节上线失败）。每次 push 前确认 nav 引用的 `.md` 都已提交。
...
- **纯静态看**：`mkdocs build`（默认出 `site/`），再 `cd site && python -m http.server 8000`。
- MkDocs 站点**必须**通过 `mkdocs serve` 或 `python -m http.server` 看，**别直接双击 `site/index.html`**（`file://` 下资源路径会错，见第八节）。
...
- **加一条书摘**：直接编辑 `docs/index.md` 在末尾追加；或新建 `书名/章节.md`，再到 `mkdocs.yml` 的 `nav:` 加一行。
- **本地预览**：`mkdocs serve`（默认 `:8000`）。
- **部署**：`git push` → Actions → `gh-deploy` 到 `gh-pages` → 发布 `digest.hanvon.top`。
...
所以 `hexo d` 把 `public/` 推到**用户页仓库的 `main`**——这就是前面说的「用户页成品就在 main」。`source/CNAME`（`blog.hanvon.top`）构建后自动落到 `public/` 根，GitHub Pages 据此绑定自定义域名。

> 想也给它接 CI 自动部署？可以参考我写的 [从零搭建多站点 MkDocs →](建站教程-从零搭建多站点MkDocs.md)，给 Civil_Blog 加一个 `hexo g` + actions-gh-pages 的工作流。但用户页默认就是 main，直接 `hexo d` 最简单，所以我目前保留手动部署。
...
| 清理缓存 | （一般无需；必要时删 `site/`） | （一般无需；必要时删 `resources/` 或 `hugo --gc`） | `hexo clean` |
...
| 部署上线 | `git push`（Actions 跑 `gh-deploy` → `gh-pages`） | `git push`（Actions → `gh-pages`） | `hexo deploy` / `hexo d`（手动推 `main`） |
| 新建内容 | 手写 `docs/*.md` + 改 `nav` | 手写 `data/*.yaml` 或 `.md` | `hexo new "标题"` |
...
1. **别直接双击 `site/` 或 `public/` 里的 `index.html` 用 `file://` 打开。**
   MkDocs / Hugo / Hexo 生成的资源路径是绝对路径（如 `/assets/css/...`）。`file://` 下浏览器会把 `/` 当成磁盘根，结果 CSS / JS 全部 404，页面「裸奔」。正确做法是用 `mkdocs serve` / `hugo server` / `hexo s` 或 `python -m http.server`（从产物目录起服务）。

2. **Hexo 同理。**
   尤其开了绝对资源路径或 abbrlink 短链后，直接 `file://` 也可能缺资源。统一用引擎自带 `server` 或 `python -m http.server` 最稳。
...
- **civil_wiki / civil_digest（MkDocs）**：加 `docs/*.md` → `mkdocs serve` 本地看 → `git push` 上线（Actions `gh-deploy` → `gh-pages` 项目页）。
