...
1. 在 GitHub 新建仓库（例如 `your-username/wiki`）。
2. 本地仓库绑定远端并推送：

   ```bash
   cd D:\WorkSpace\Note\wiki
   git remote add origin https://github.com/your-username/wiki.git
   git branch -M master
   git push -u origin master
   ```

3. 在 `mkdocs.yml` 里把占位地址换成真的：

   ```yaml
   site_url: https://your-username.github.io/wiki/
   repo_url: https://github.com/your-username/wiki
   edit_uri: edit/master/docs
   ```

   > 当前 `site_url` 已设为 `https://your-username.github.io/wiki/`（占位），所以后面的社交卡片、Sitemap 都能正常生成；你只需替换用户名即可。

4. 仓库 `Settings → Pages → Source` 选 "GitHub Actions"，仓库会自动运行 `.github/workflows/ci.yml` 部署到 `gh-pages` 分支。
...
1. 你去 GitHub 创建仓库，替换 `mkdocs.yml` 里的 `your-username`。
2. 推送后确认 GitHub Actions 成功，访问 `https://your-username.github.io/wiki/`。
3. 后续真实笔记可以按 `docs/领域/主题/标题.md` 放入，导航和标签会自动变得饱满。
4. 再有改造时，直接新增 `docs/博客/博客改造日志/YYYY-MM-DD-标题.md`。
