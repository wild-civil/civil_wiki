...
> 制胜一招是"**换板对比 + 串口诊断**"：开发板用 `Main_uart_diag.c` 在进睡前打印 `ENTER SLEEP`、唤醒打印 `WAKEUP`，先确认芯片**确实睡了**（排除伪睡/自唤醒），再把矛头指向**自己板子的板级外围电路**。
...
- 对 KeyGo 的用法：把上述数据当作**量级参考锚点**——开发板 35 µA 介于官方 15 µA 与 Demo 18 µA 之间（合理），自焊板 4.4 mA 远超 → 板级 IC 问题；若你的板子睡眠态远高于 35 µA，优先走 3.2 的 SOP 而非继续微调配比。
