...
!!! info "真实调试经历见「调试踩坑记录」（已校正根因）"
    KeyGo 早期确实因低功耗压不下去、误以为是代码问题折磨了 4 天，最后换淘宝开发板对比（开发板 35 µA vs 自焊板 4.4 mA）才锁定硬件。**但经固件 memory 校正，真正的元凶是 GPIO 浮空（PB0 KeyPower PMOS / PB4–7 TMUX1112 控制脚浮空让外部负载持续通电），不是 XC6206**——XC6206 山寨是上一段的独立坑，别张冠李戴。完整时间线与排查方法论见 [KeyGo 调试踩坑记录](KeyGo-调试踩坑.md#1-深睡电流降不下去gpio-浮空让外设持续通电血泪-4-天)。
