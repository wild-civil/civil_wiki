...
| 工况 | 实测电流 | 备注 |
|------|----------|------|
| 深度睡眠 | __ µA | CH582M 标称最低约 0.3 µA，待实测 |
| 间歇广播 | __ µA（平均） | 广播间隔 __ ms |
| 连接保持 | __ mA（平均） | 连接间隔 __ ms |
| 峰值（TX @ 0 dBm） | __ mA | — |

!!! tip "降低平均电流的三板斧"
    1. **拉长广播/连接间隔**：间隔翻倍，连接态平均电流近似减半，但解锁延迟变大——需在"手感"与"续航"间折中。
    2. **睡眠期间关外设时钟与 LDO 旁路**：见第 4 节供电选型。
    3. **RSSI 采样不要在连接事件外另起高频定时**：复用连接事件节拍，避免额外唤醒。

> 待补：KeyGo 实际采用的广播/连接间隔、睡眠策略、以及"靠近即解锁"端到端延迟实测。
...
!!! info "真实调试经历见「调试踩坑记录」"
    第 1 节"山寨料静态电流差 300 倍"在实战里真的发生过：曾因低功耗压不下去、误以为是代码问题折腾 4 天，最后换淘宝开发板对比才发现是板上 XC6206 的暗电流顶高了底电流。完整时间线与排查方法论见 [KeyGo 调试踩坑记录](KeyGo-调试踩坑.md#1-ldo-暗电流淹没了低功耗血泪-4-天)。
