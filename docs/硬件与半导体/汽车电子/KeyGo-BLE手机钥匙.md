...
> 待补：KeyGo 的固件版本演进（v3.5.7 等）、当前已知 Bug 清单、与 CH582M 裸片/模组选型说明。
...
!!! warning "深度睡眠 35 µA 的来历与坑"
    这 35 µA 是**开发板在修好 GPIO 浮空之后**的数字。自焊板在修 GPIO 前深睡高达 **4.4 mA**——根因是 PB0(KeyPower PMOS)/PB4–7(TMUX1112) 控制脚浮空，让外部负载持续通电，**不是 LDO**。完整复盘见 [KeyGo 调试踩坑记录](KeyGo-调试踩坑.md#13-根因gpio-浮空导致外部负载持续导通不是-ldo)。
...
> 待补：KeyGo 量产固件的广播/连接间隔、睡眠策略、"靠近即解锁"端到端延迟，以及连接保持/峰值 TX 的最终实测。
...
> 待补：KeyGo 采用的 NFC 芯片/卡片型号、与 CH582M 的接口（SPI？独立 NFC 控制器？）、门限感应区位置与读卡距离实测。
...
> 待补：KeyGo 实际踩到的 XC6206 具体现象（电压跌落曲线？断连日志？）、所用渠道/丝印，以及换料后的对比。
...
> 待补：KeyGo 最终选用的 LDO 型号、实测压差/纹波/静态电流，以及与原山寨 XC6206 的续航对比数据。
...
> 待补：KeyGo 实际落地的抗中继措施（是否引入 UWB？RSSI 阈值本身能否抗中继？）、以及相应测试结果。
