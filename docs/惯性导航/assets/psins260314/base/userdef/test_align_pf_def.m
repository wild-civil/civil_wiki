function out = test_align_pf_def(tag, varargin)
% See also  test_align_ukf, kfinit, kffk, kfhk, kfplot.
global glv psinsdef
switch tag
	case psinsdef.kfinittag,
        [nts, imuerr, N] = setvals(varargin);
        kf.fx = @afamodel;
        kf.Qk = diag([imuerr.web; imuerr.wdb])^2*nts;  kf.Gammak = 1;
        kf.sQ = diag(sqrt(kf.Qk));
        kf.Rk = diag([0.1;0.1;0.1])^2;
        kf.sR = diag(sqrt(kf.Rk));
        kf.Pxk = diag([[10;10;180]/20*glv.deg; [1;1;1]])^2;
        kf.Hk = [zeros(3), eye(3)];
        kf.Npar = N;
        kf.particle = zeros(6,N) + chol(kf.Pxk)*randn(6,N);
        kf.weight = zeros(1,N);
        out = kf;
