...
%   avpn = inspure_enu(imuadderr(imu,imuerrset(0,0,0,000)), avp0, 'V');
%   avpcmpplot_polar(avp, avptrans(avpn,'n2e'), 'e');
%   avpn1 = inspure_enu(imu, avp0, 'V');  avpcmpplot_polar(avpn, avpn1, 'n');
%
% See also  insinit_enu, insupdate_enu, inspure, inspure_ecef, inspure_grid.
...
global glv
    ...
    ins = insinit_enu(avp0, ts);  vn0 = avp0(4:6); pos0 = avp0(7:9);
    len = length(imu);    avp = zeros(fix(len/nn), 10);  wUdlon=avp(:,1:3);
    ki = timebar(nn, len, 'ENU-frame pure inertial navigation processing.');
    for k=1:nn:len-nn+1
        k1 = k+nn-1;
        wvm = imu(k:k1, 1:6);  t = imu(k1,end);
        ins = insupdate_enu(ins, wvm);
        ...
        avp(ki,:) = [ins.avp; t]';
        wUdlon(ki,:) = [ins.wUdlon; t]';
        ki = timebar;
    end
    if nargin<4, isfig=1; end
    if isfig==1,
        insplot_polar(avp,'n');
%         myfig; plot(wUdlon(:,end), wUdlon(:,1:2)/glv.dps);  xygo('w_U,dlon / deg/s');
    end
