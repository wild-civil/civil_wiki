function att = atttrans(att, typ, fireAng)
% Batch convertion for Euler attitude angles,
% yaw->pitch->roll(@RFU) to pitch->yaw->roll(@FUR), or conversely.
%
% Prototype: att = atttrans(att, typ, firAng)
...
% 01/08/2023
    ...
    if typ==0, % yaw-pitch-roll to pitch-yaw-roll
        Cnb = [ cj.*ck-si.*sj.*sk, -ci.*sk,  sj.*ck+si.*cj.*sk, ...
               ...
               -ci.*sj,             si,      ci.*cj           ];
        if nargin>=3,  Cnb = m3xm3(rxyz(-fireAng,'z'), Cnb);  end
