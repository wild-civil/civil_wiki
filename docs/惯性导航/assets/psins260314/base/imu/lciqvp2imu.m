function [imu, qvp] = lciqvp2imu(qvp, pos0, A0)
% Transform LCI AVP to FUR-IMU for launch vehicle use.
%
% Prototype: [imu, qvp] = lciqvp2imu(qvp, pos0, A0)
% Inputs: qvp - LCI-frame QVP =[q1,q2,q3, vx,vy,vz, x,y,z, t],
%                               q1,q2,q3-quaternion vectro part,
...
%                         x-forward, y-upward, z-rightward for vel&pos
%
% See also  lciqp2qvp, lciap2avp.

% Copyright(c) 2009-2025, by Gongmin Yan, All rights reserved.
% Northwestern Polytechnical University, Xi An, P.R.China
% 13/05/2025
global glv
    if qvp(1,end)>0, qvp=[[2*qvp(1,1:9)-qvp(2,1:9),0]; qvp];  end  % make sure from t=0
    Cn0a = a2mat([0;A0;0]);
    [re0,Ce0n0] = blh2xyz(pos0); Ce0n0 = CenNUE(Ce0n0); Ce0a = Ce0n0*Cn0a;  ra0 = Ce0a'*re0;
    qab0 = q32q4(qvp(1,1:3)');  va0 = qvp(1,4:6)';  pa0 = qvp(1,7:9)';  t0 = qvp(1,end);
    len = length(qvp);  imu = zeros(len-1,7);
    ki = timebar(1, len, 'Translate LCI AVP to IMU.');
    for k=2:len
        qab = q32q4(qvp(k,1:3)');  va = qvp(k,4:6)';  pa = qvp(k,7:9)';  t = qvp(k,end);
