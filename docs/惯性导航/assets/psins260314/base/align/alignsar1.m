function [att, att0, res, phi, vnerr] = alignsar1(av, pos, isppm, isfig)
...
% Example
%    ts = 0.001;  T = 150;  
%    [imu0, att0, T1T2, avp0] = imustatictp(avpset([1;2;0]*glv.deg,0,glv.pos0,0), ts, T, 30*glv.dps, 30*12*10*glv.deg);
%    t1 = 30;  t2 = 100;  imu = imuadderr(imu0, imuerrset(0.01,10,0,0));
%    att00 = getat(att0,t1-ts);  imu1 = cut(imu,t1,t2);
%    avp = inspure(imu1, [aaddphi(att00(1:3),[1;2;3]*glv.min);glv.pos0], 'O', 1);
%    [att1sar, att0sar] = alignsar1(avp, glv.pos0, 0, 1);
%
% See also  alignsar, alignsars, alignvn, aligni0, alignsbtp, insupdate.
