...
Z-Y-X 组合出来的旋转矩阵，本项目统一用**被动旋转**约定（与 [坐标变换与旋转矩阵（推导入门）](../00_前置数学基础/坐标变换与旋转矩阵.md) 第一章一致，也是课本主流写法）：

- **nav→body**（推导入门已推导）：$C_n^b = R_x(\phi)\,R_y(\theta)\,R_z(\psi)$，矩阵**从右往左读**，最先发生的旋转（Z）放在最右。
- **body→nav（本篇 §四 DCM 段记为 $R_{nb}$，与这里的 $C_b^n$ 是同一物体、两种记法）**：旋转矩阵正交，直接取转置即可： $C_b^n = (C_n^b)^T = R_z(-\psi)\,R_y(-\theta)\,R_x(-\phi)$。即"原路倒着转、角度取负"——相对 $C_n^b$ 顺序反过来（Z 到了最左），正是 [推导入门](../00_前置数学基础/坐标变换与旋转矩阵.md) 第五节的 $C_b^n$。两种写法**数值完全相等**（被动正角 $R^{pass}(\alpha)$ 与主动形式 $R^{act}(\alpha)$ 互为转置，也等于 $R^{pass}(-\alpha)$），只是约定不同，不要混用。
...
> 此矩阵与 [推导入门](../00_前置数学基础/坐标变换与旋转矩阵.md) 第五节逐元素一致，可直接当参考表用。
...
!!! note "本篇是被动旋转，别用主动画面去读列"
    ...
    **「旧轴在新系里怎么读」**，而不是「旧轴被搬到哪」——后者是**主动旋转**的画面，矩阵差一个转置。
    两套说法的并列对比见 [坐标变换与旋转矩阵 · 0.4b 岔路口](../00_前置数学基础/坐标变换与旋转矩阵.md)。
...
👉 完整三步推导见 [坐标变换与旋转矩阵 · 第零节](../00_前置数学基础/坐标变换与旋转矩阵.md)；可拖动的演示见 [旋转矩阵三步推导 · 交互演示](../assets/旋转矩阵_三步推导_交互.html){:target="_blank"}
...
> 答案都藏在上面。第 1、3、5 题答不清，建议回看 [01 坐标系与变换](01_坐标系与变换.md) 与 [数学地基：矩阵、概率与协方差](../00_前置数学基础/数学基础.md) 的旋转矩阵小节。
...
- 数学地基：[矩阵、概率与协方差](../00_前置数学基础/数学基础.md)（旋转矩阵、四元数乘法都在这里）
