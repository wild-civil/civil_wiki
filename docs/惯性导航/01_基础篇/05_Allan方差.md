...
![本篇一图流](../assets/05一图流_Allan方差.svg)
...
> 关键经验：**最低点 τ 就是"BI 主导区"**——这也是 04 篇 [04 零偏漂移 vs 随机游走](04_IMU误差模型.md#random-walk) 那张 SVG 提到的交点。
...
- 数学地基：[矩阵、概率与协方差](../00_前置数学基础/03_数学基础.md)
...
- 资产：所有出图与脚本都在 wiki 的 `assets/` 目录（目录本身无页面，请本地浏览）：`gen_allan_matlab.m` / `gen_allan_py.py` / `allan_curve_matlab.png` / `allan_curve_python.png` —— 双轨可复现
