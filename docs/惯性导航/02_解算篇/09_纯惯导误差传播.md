...
4. **三方互证**：PSINS `insupdate` 循环（工业标准实现）跑同场景——详见下文「PSINS 版对照」：**陀螺零偏→线性发散项三方 <5% 一致（1.35 / 1.35 / 1.28 km）**，全组合量级一致（~6-10 km），差异来源明确（PSINS 含哥氏/曲率/椭球全项 + 加计耦合放大）。
...
> C 版是教学简化实现；**PSINS 是工业标准实现**。同一场景用 PSINS 的 `insinit` + `insupdate`（显式循环，不用黑盒 `inspure`）跑一遍，看两者怎么对应、数值差在哪——这就是 M3 后拆解 PSINS 系列的"预告片"。

**脚本**：[../assets/gen_psins_pure_demo.m](../assets/gen_psins_pure_demo.m)（需 MATLAB + PSINS 工具箱）
...
> ⚠️ 脚本里踩过的坑（已注释）：① `avperrset` 的 phi 单位是 **arcmin** 不是 deg；② PSINS pos 单位 **rad**，转 m 乘 `glv.Re`（不是 1e5）；③ 别手动改 `imuerr.eb/db` 字段（破坏 PSINS 内部结构 → NaN），要单轴注入就**直接改 imu 数据列**（`imu(:,2)` 加角增量零偏、`imu(:,4)` 加比力零偏）；④ `imuerrset` 默认三轴同值，与 C demo 单轴注入不可比。
...
- **下一篇**：[10 卡尔曼滤波基础](../03_组合导航篇/10_卡尔曼滤波基础.md)——纯惯导必发散，引入量测修正 → KF 五方程（协方差传播有了"修正项"）
