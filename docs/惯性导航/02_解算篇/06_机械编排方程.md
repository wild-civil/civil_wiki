...
- $\boldsymbol C_b^n$：把 **body 系向量**投影到 **NED 导航系**的旋转矩阵（$v_n = C_b^n v_b$）。⚠️ 注意区分两个方向：**坐标投影**是 body→NED；而"坐标系变换"（把 NED 系转到 body 系）是它的转置 $C_n^b = (C_b^n)^T$——固件 `q2r` 给的是后者（见第六节符号方向坑）。
