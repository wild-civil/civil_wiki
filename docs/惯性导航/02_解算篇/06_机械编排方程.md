...
📚 **牛小骥 I2NAV 讲义 PDF（在线预览）**：本系列涉及的 5 讲已收录在 wiki 的 `assets/I2NAV组合导航讲义/` 目录（目录本身无页面，请本地浏览），以下各讲 PDF 可直接在线阅读：
...
> 牛小骥讲义的符号定义是业内比较清晰的，直接采用：

- **上标/下标约定**：$\boldsymbol v^n$ 表示速度在 **n 系（导航系）投影**；$\boldsymbol f^b$ 表示比力在 **b 系（机体系）投影**
- $\boldsymbol C_b^n$：**b 系 → n 系**的**坐标**变换矩阵（把 body 向量转到 NED的姿态矩阵）；**n 系 → b 系**的**坐标系**变换矩阵
- $\boldsymbol\omega_{ie}$：**地球自转角速度** = e 地球系相对 i 惯性系（i 系为参考；记法见下方约定），常量 ≈ 7.29e-5 rad/s
- $\boldsymbol\omega_{en}$：**导航系相对地球的旋转**（载体在地球表面移动引起），$\boldsymbol\omega_{in} = \boldsymbol\omega_{ie} + \boldsymbol\omega_{en}$ 是"导航系相对惯性系的旋转"
- **四个坐标系**：i 惯性系（不转）、e 地球系（随地球转）、n 导航系、b 机体系。记法 $\boldsymbol\omega_{ab}$ = **b 系相对 a 系的角速度**（第二个字母为目标系：$\omega_{ie}$=e 相对 i、$\omega_{en}$=n 相对 e、$\omega_{in}$=n 相对 i）；$\boldsymbol\omega_{ie}^n$ 表示它在 n 系的投影。

**三个量在 n 系的分量（NED 为主，附 ENU 对照；NED 列序 $[N,E,D]$，ENU 列序 $[E,N,U]$）**：

| 量 | NED（北-东-地） | ENU（东-北-天） |
|---|---|---|
| $\boldsymbol\omega_{ie}^n$ 地球自转 | $[\,\omega\cos L,\ 0,\ -\omega\sin L\,]$ | $[\,0,\ \omega\cos L,\ +\omega\sin L\,]$ |
| $\boldsymbol\omega_{en}^n$ 运移 transport | $[\,\dfrac{V_E}{R_N},\ -\dfrac{V_N}{R_M},\ -\dfrac{V_E\tan L}{R_N}\,]$ | $[\,-\dfrac{V_N}{R_M},\ \dfrac{V_E}{R_N},\ +\dfrac{V_E\tan L}{R_N}\,]$ |
| $\boldsymbol\omega_{in}^n$ 导航系总旋转 | 上两行逐分量相加 | 同左 |

**典型数值**：$\omega=7.2921151467\times10^{-5}\ \text{rad/s}\approx 15.04\ °/h$，故 $\omega_{in}\approx\omega_{ie}\approx15°/h$，运动时才叠加 $\omega_{en}$（量级 $10^{-4}\sim10^{-3}\ °/s$，相对 15°/h 可忽略）。

**坐标对照**：ENU 与 NED 仅"天向 U ↔ 下向 D"取反、East/North 分量顺序交换，所以 $\omega_{ie}^n$ 天向分量 ENU 为 $+\omega\sin L$、NED 为 $-\omega\sin L$，运移天向项同理变号。这正是 PSINS 工程用 ENU、本项目（H743/ESKF）用 NED 时 `wnie/wnen/gcc` 都要翻符号的根源（对照 [04 拆解PSINS篇 · 10 机械编排](../../04_拆解PSINS篇/PSINS_STM32嵌入式/10_机械编排与惯导解算.md) 的 ENU 代码）。
...
- 下一篇：[07 姿态更新算法](07_姿态更新算法.md)——欧拉法/四元数积分/Bortz 旋转向量/RK4、圆锥与划桨效应（牛小骥 I2NAV 讲义式 134-135 + 6 方法实测对比）
