...
??? note "🧪 运行方式（Python 或 MATLAB，二选一即可）"
    ...
    5. **可视化**：生成 `miniinsplot_truth.png`（真值轨迹）、`miniinsplot_dr.png`（DR 解算轨迹）、`miniinsplot_cmp.png`（真值 vs DR 同图叠加）、`miniavpcmpplot_dr.png`（DR vs 真值残差）、`miniinsplot3d_cmp_dr.png`（**3D 轨迹对比，新增**）。
