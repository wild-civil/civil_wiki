...
| `blog2.hanvon.top` | `wild-civil/wild-civil.github.io` | Hexo | 用户站点 | 已上线 ✅ |
| `digest.hanvon.top` | `wild-civil/digest`（待建） | 任意静态站 | 项目页 | 待建 |
| `works.hanvon.top` | `wild-civil/works`（待建） | 任意静态站 | 项目页 | 待建 |
...
### 3. 书摘 `digest.hanvon.top`（待建）

1. 本地建仓库，选用任意静态生成器（MkDocs / Hexo / VitePress 等）
2. 源码放 `CNAME` 写 `digest.hanvon.top`
3. 推到 GitHub 新建仓库 `digest`，配 Pages + Actions 部署
4. DNS 加 `digest` CNAME → `wild-civil.github.io`
5. 仓库 `Settings → Pages → Custom domain` 填 `digest.hanvon.top`

### 4. 作品 `works.hanvon.top`（待建）

同书摘，仓库名 `works`，子域名 `works.hanvon.top`。
