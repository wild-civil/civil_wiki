...
u8 mcu_init_gpscfg=0;

u8 Rx2_data[120], Rx2_data1[120];                     
u8 Rx2_complete=0;                 
u16 Length2 =0;
...
void mcu_init(void)          // ★ 初始化所有硬件
{
	RCC_Configuration();     //时钟初始化
	GPIO_Configuration();    //端口初始化   
	USART1_Configuration(0); //COM1 for USB输出串口初始化
	USART2_Configuration();  //COM2 for GPS串口初始化
	SPI1_Configuration();    //SPI for MPU9250初始化
	TIM2_Configuration();    //定时中断频率 ★ 100Hz，读MPU9250
	TIM3_Configuration();    //定时中断频率 ★ 50Hz，读气压
	Init_MPU9250();          //MPU9250初始化

	MS561101BA_RESET();      //气压传感器初始化
	Delay(100000);
...
void USART1_Configuration(u8 gpscfg)
{
	...
	mcu_init_gpscfg = gpscfg;
	USART_InitStructure.USART_BaudRate = mcu_init_gpscfg ? 115200 : 460800;
...
void USART1_DIA_OUT_Configuration(void)
{
   ...
   DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)&outFrame; //Usart1_out_DATA;
   DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
   DMA_InitStructure.DMA_BufferSize = (uint16_t)sizeof(outFrame); //Usart1_out_Length;
...
void TIM2_Configuration(void) 
{
   ...
   TIM_TimeBaseStructure.TIM_Period = 99;//9;//  计数器变化1对应0.1ms  
