function out = test_SINS_OD_def(tag, varargin)
...
switch tag
	case psinsdef.kfinittag,
        ...
        kf.Pxk = diag([davp; imuerr.eb; imuerr.db; davp(7:9); dinst([1,3]); dKod; dT]*10)^2;
        kf.Hk = zeros(3,22); kf.Hk(1:3,7:9) = eye(3); kf.Hk(1:3,16:18) = -eye(3);
        out = kf;
    case psinsdef.kffktag,
        % SINS/OD: INS(15)+dpitch/dKod/dyaw(3)=18
        ins = varargin{1}{1};
    ...
    case psinsdef.kfplottag,
        [xkpk, avp, xkpk] = setvals(varargin);
