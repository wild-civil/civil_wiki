function [att, attk, xkpk, kf] = alignvntpv20(imu, qnb, pos, phi0, imuerr, wvn, T1T2, isfig)
...
%    [phiE,phiN,phiU, dvE,dvN,dvU, ebx,eby,ebz, dbx,dby,dbz, dvE20,dvN20]'.
%
% Prototype: [att, attk, xkpk, kf] = alignvntpv20(imu, qnb, pos, phi0, imuerr, wvn, T1T2, isfig)
% Inputs: imu - IMU data
...
%         isfig - figure flag
% Outputs: att - attitude align result
%         attk, xkpk - for debug
%
% See also  alignvntp, alignvn.
...
global glv
    ...
    att = attk(end,1:3)';
    resdisp('Initial align attitudes (arcdeg)', att/glv.deg);
...
function kf = avnkfinit(nts, pos, phi0, imuerr, wvn)
	...
	kf.Qk = diag([imuerr.web; imuerr.wdb; zeros(6,1); zeros(2,1)])^2*nts;
    kf.Gammak = 1;
	kf.Rk = diag(wvn)^2/nts;
	kf.Pxk = diag([phi0; [1;1;1]; imuerr.eb; imuerr.db; [0.1;0.1]])^2;
	Ft = zeros(14); Ft(1:3,1:3) = askew(-wnie); kf.Phikk_1 = eye(14)+Ft*nts;
	kf.Hk = [zeros(3),eye(3),zeros(3,6), zeros(3,2)];
    kf = kfinit0(kf, nts);
...
global glv
    ...
    subplot(212); plot(t, xkpk(:,13:14)); xygo('dv');  title(sprintf('%.4f, ',xkpk(end,13:14)));
