function [av, xkpk] = alignsarkf13(imu, avp0, imuerr, phi0, wvn, lvb)
% 13-state single axis rotation initial alignment by KF and using vn as
% measurement. Usually rotated by z/Up axis and with z-axis gyro scale
% factor estimated.
%
% Prototype: [av, xkpk] = alignsarkf(imu, avp0, imuerr, phi0, wvn)
...
%         lvb - rotation center lever-arm w.r.t IMU
% Outputs: av - att & vn out:
%          xkpk - KF xk & Pk
%
% Example
%   ierr = imuerrset(0.01,1000,0.001,10, 0,0,0,0, [0.1;1;100]);
%   [av, xkpk] = alignsarkf13(imu, [att0;pos0], ierr, [10;10;60]*glv.min, [1;1;1]*0.1);
%
% See also  alignsarkf10, alignsar, alignvn, alignvnkf43, sysclbt, imuerrset, alignscat.
...
global glv
    ...
    timebar(nn, len, sprintf('13-state single axis rotation initial alignment, '));
    ...
    for k=1:nn:len-nn
        ...
        if t1s>(0.2-ts/2)  % kf measurement update every 1 second
            ...
            dkgzz = dkgzz+afa*kf.xk(13);  kf.xk(13) = afa1*kf.xk(13);
        end
...
global glv
    kf.Qt = diag([ ierr.web; ierr.wdb; [0;0;0]; [1;1;1]*glv.ug/sqrt(glv.hur); ierr.dKg(2,2)/glv.ppm*glv.ppmpsh ])^2;  % NOTE: dKgyy=random walk of dKgzz
    kf.Rk = diag(wvn)^2;
    kf.Pxk = diag([ phi0; [1;1;1]*1.1; [ierr.eb(1:2);0]; ierr.db; ierr.dKg(3,3) ])^2;
    kf.Hk = [zeros(3),eye(3),zeros(3,7)];
    kf = kfinit0(kf, ts);
...
function Ft = getFt(fb, wb, Cnb, wnie)   % kffk
    ...
    %        1   4     7    10    13       
    %states: fi  dvn   eb   db    dKgzz
    Ft = [  -wX  o33  -Cnb  o33  -[0;0;wz]
             fX  o33   o33  Cnb   o31
             zeros(7,13) ];
...
global glv
    ...
    myfigure
    xk = xkpk(:,1:13); t = xkpk(:,end);
    subplot(521), plot(t, xk(:,1:3)/glv.min); xygo('phi');
    subplot(523), plot(t, xk(:,4:6)); xygo('dv')
    subplot(525), plot(t, xk(:,7:9)/glv.dph); xygo('eb');
    subplot(527), plot(t, xk(:,10:12)/glv.ug); xygo('db');
    subplot(529), plot(t, xk(:,13)/glv.ppm); xygo('dKgzz');
    xk = sqrt(xkpk(:,14:end-1));
    subplot(522), plot(t, xk(:,1:3)/glv.min); xygo('phi');
    subplot(524), plot(t, xk(:,4:6)); xygo('dv')
    subplot(526), plot(t, xk(:,7:9)/glv.dph); xygo('eb');
    subplot(528), plot(t, xk(:,10:12)/glv.ug); xygo('db');
    subplot(5,2,10), plot(t, xk(:,13)/glv.ppm); xygo('dKgzz');
