function [att0, attk, xkpk] = alignvntp(imu, qnb, pos, phi0, imuerr, wvn, T1T2, isfig)
% SINS two-position initial align uses Kalman filter with vn as measurement.
% Kalman filter states: 
%    [phiE,phiN,phiU, dvE,dvN,dvU, ebx,eby,ebz, dbx,dby,dbz]'.
%
% Prototype: [att0, attk, xkpk] = alignvntp(imu, qnb, pos, phi0, imuerr, wvn, T1T2, isfig)
% Inputs: imu - IMU data
...
%	[imu, att, T1T2] = imustatictp(avp0, 0.01, 300, -10*glv.dps, 180*glv.deg, imuerr);
%	phi = [.5; .5; 5]*glv.deg;
%	wvn = [0.01; 0.01; 0.01];  % inspure(imu, avp0, 'O');
%	[att0, attk, xkpk] = alignvntp(imu, avp0(1:3)', avp0(7:9)', phi, imuerr, wvn, T1T2);
%
% See also  alignvntpv20, alignvn.
